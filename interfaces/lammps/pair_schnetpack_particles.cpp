...
PairSCHNETPACK::~PairSCHNETPACK(){
  if (allocated) {
    ...
    // Additions for Particles systems
...
void PairSCHNETPACK::compute(int eflag, int vflag){
  ...
  // Additions for particles systems
  torch::Tensor particletype_tensor = torch::zeros({nlocal}, torch::TensorOptions().dtype(torch::kInt64));
  ...
  // Additions for particles systems
  auto particletype = particletype_tensor.accessor<long, 1>();
  ...
  for(int ii = 0; ii < inum; ii++){
    ...
    // Additions for particles systems
    particletype[itag-1] = itype-1;  // Python types are 0-based
    std::cout << "DEBUG: ii " << ii << "\n";
    std::cout << "DEBUG: i " << i << "\n";
    std::cout << "DEBUG: itag " << itag << "\n";
    std::cout << "DEBUG: tag2i " << tag2i[itag-1] << "\n";
  ...
  // Additions for particles systems  
  std::vector<std::string> requested_lists = {"bonded", "nonbonded"};
  int n_bond_types = atom->nbondtypes;  // number of bond types, speciied in the data
  int n_bonds = atom->nbonds;           // number of bonds, speciied in the data
  std::cout << "DEBUG: n_bond_types " << n_bond_types << "\n";
  std::cout << "DEBUG: n_bonds " << n_bonds << "\n";

  int **bonds = atom->bond_atom;        // for each atom (column), lists the atoms it is bonded with (row)
  int *num_bond = atom->num_bond;       // for each atom it specifies how many bonds it has
  for(int iat = 0; iat < nlocal; iat++){
    std::cout << "DEBUG: Atom - i " << iat << "\n";
    std::cout << "DEBUG: ilist " << ilist[iat] << "\n";
    std::cout << "DEBUG: tag " << tag[iat] << "\n";
    std::cout << "DEBUG: num_bond " << num_bond[iat] << "\n";
      for(int ib = 0; ib < num_bond[iat]; ib++){
        std::cout << "DEBUG: Bonded with (tag) " << bonds[iat][ib] << "\n";
        std::cout << "DEBUG: also called (i) " << tag2i[bonds[iat][ib]-1] << "\n";
      }
  }
  
  int n_bonds_x2 = n_bonds * 2;        // size of the idx_of_bonds list to send to schnet
  torch::Tensor idx_of_bonds_tensor = torch::zeros({n_bonds_x2, 2}, torch::TensorOptions().dtype(torch::kInt64));
  auto idx_of_bonds = idx_of_bonds_tensor.accessor<long, 2>();
  int i_bond_pairs = 0;
  for(int iat = 0; iat < nlocal; iat++){
    for(int ib = 0; ib < num_bond[iat]; ib++){
      idx_of_bonds[i_bond_pairs][0] = tag[iat];
      idx_of_bonds[i_bond_pairs][1] = bonds[iat][ib];
      i_bond_pairs = i_bond_pairs + 1;
    }
  }

  // NEXT TODO = loop over edge_tensor (see below) and keep only pairs 
  //             not included within idx_of_bonds
  ...
  // Additions for particles systems
  input.insert("_particle_types", particletype_tensor.to(device));
  input.insert("_idx_of_bonds", idx_of_bonds_tensor.to(device));
  //input.insert("_idx_non_bonds", idx_non_bonds_tensor.to(device));
  //input.insert("_idx_of_inter", idx_of_inter_tensor.to(device));
  //input.insert("_idx_of_intra", idx_of_intra_tensor.to(device));
  ...
  torch::Tensor total_energy_tensor = output.at("intermol_energy").toTensor().cpu();
  ...
  if(debug_mode){
    ...
    std::cout << "intermol_energy: " << total_energy_tensor << "\n";
  }
