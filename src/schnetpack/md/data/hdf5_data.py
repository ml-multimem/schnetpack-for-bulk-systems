...
class HDF5Loader:
    ...
    def _load_molecule_data(self):
        ...
        entry_start = entry_stop
        entry_stop += entry_width
        self.properties["images"] = raw_positions[
            :, :, entry_start:entry_stop
        ].reshape(self.entries, self.n_replicas, self.total_n_atoms, 3)
        ...
        if structures.attrs["has_cells"]:
            ...
            self.properties[properties.cell] = raw_positions[
                :, :, entry_start:entry_stop
            ].reshape(self.entries, self.n_replicas, 1, 3, 3)
    ...
    def get_images(
        ...
        return self.get_property(
            "images", atomistic=True, mol_idx=mol_idx, replica_idx=replica_idx
        )
...
class HDF5ParticlesLoader:
    """
    Class for loading HDF5 datasets written by the FileLogger for Particles objects. By default, this requires at least a 
    MoleculeStream to be present. PropertyData is also read by default, but can be disabled.
    ...
    def __init__(
        ...
        self.skip_initial = skip_initial
        self.data_groups = list(self.database.keys())
        self.n_atoms_per_mol = n_atoms_per_mol
        self.properties = {}
    ...
    def _load_molecule_data(self):
        ...
        self.n_atoms_mol = np.repeat([self.n_atoms_per_mol], self.n_molecules)

        # Set up molecule ranges
        self.molecule_range = np.pad(np.cumsum(self.n_atoms_mol), (1, 0), mode="constant")
        ...
        if structures.attrs["has_cells"]:
            ...
            self.properties[f"{properties.stress}_system"] = raw_positions[
                :, :, entry_start:entry_stop
            ].reshape(self.entries, self.n_replicas, 1, 3, 3)  #unsure about the 3rd dimension
        else:
            self.properties[properties.cell] = None
    ...
    def get_positions(
        ...
        Returns:
            np.array: N_steps x N_atoms x 3 array containing the atom positions of the simulation in internal units.
        """
        pos = np.zeros((self.total_entries, self.total_n_atoms, 3))
        for i in range(self.n_molecules):
            pos[:, i*self.n_atoms_mol[i]:(i+1)*self.n_atoms_mol[i] ,:] =  self.get_property(
                properties.R, atomistic=True, mol_idx=i, replica_idx=replica_idx
            )
        return pos
    ...
    def get_potential_energy(
        ...
        potential_energy = [self.get_property(
            energy_key, atomistic=False, mol_idx=mol_idx, replica_idx=replica_idx
            ) for i in range(self.n_molecules)]

        tot_potential_energy = np.sum(np.array(potential_energy), axis = 0)
    ...
    def get_temperature(
        ...
        # Get the kinetic energy
        kinetic_energy = [self.get_kinetic_energy(
            mol_idx = i, replica_idx=replica_idx
        ) for i in range(self.n_molecules)]

        tot_kin_energy = np.sum(np.array(kinetic_energy), axis = 0)

        # Compute the temperature
        temperature = 2.0 / (3.0 * units.kB * self.total_n_atoms) * tot_kin_energy
    ...
    def get_pressure(
        ...
        # Compute isotropic stress #TODO check if the molecules are handled correctly
        stress = (
            np.einsum(
                "bii->b", self.get_stress(mol_idx=mol_idx, replica_idx=replica_idx)
            )
        ...
        # Compute the pressure and convert to bar
        pressure = (2.0 / 3.0 * tot_kin_energy / volume - stress) / units.bar
